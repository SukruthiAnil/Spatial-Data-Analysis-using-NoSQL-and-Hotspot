...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  // Selecting x, y, z and filtering based on min and max values
  val filteredPickupInfo = pickupInfo.select("x", "y", "z")
    .where(col("x").geq(minX) && col("y").geq(minY) && col("z").geq(minZ) &&
          col("x").leq(maxX) && col("y").leq(maxY) && col("z").leq(maxZ))
    .orderBy("z", "y", "x")

  // Grouping by x, y, z and counting pickups in each cell
  var hotCellCounts = filteredPickupInfo.groupBy("z", "y", "x")
    .count()
    .withColumnRenamed("count", "hot_cell")
    .orderBy("z", "y", "x")

  // Creating a Temporary View for the Hotcell data
  hotCellCounts.createOrReplaceTempView("HotcellCounts")

  // Calculate average of hotcells
  val averageHotCells = hotCellCounts.select(avg("hot_cell")).first().getDouble(0) / numCells
  //val avg = (hotCell_df.select("hot_cell").agg(sum("hot_cell")).first().getLong(0).toDouble) / numCells

  // Calculate Standard deviation
  val standardDeviationHotCells = scala.math.sqrt(
    hotCellCounts.withColumn("squared_cell", pow(col("hot_cell"), 2))
      .select(sum("squared_cell"))
      .first().getDouble(0) / numCells - scala.math.pow(averageHotCells, 2)
  )

  // Get all the adjacent hotcells count by comparing the x,y,z coordinates
  var adjacentHotCellCounts = spark.sql(
    """
      |SELECT h1.x AS x, h1.y AS y, h1.z AS z,
      |       sum(h2.hot_cell) AS cell_count
      |FROM HotcellCounts AS h1, HotcellCounts AS h2
      |WHERE (h2.y = h1.y+1 OR h2.y = h1.y OR h2.y = h1.y-1) AND
      |      (h2.x = h1.x+1 OR h2.x = h1.x OR h2.x = h1.x-1) AND
      |      (h2.z = h1.z+1 OR h2.z = h1.z OR h2.z = h1.z-1)
      |GROUP BY h1.z, h1.y, h1.x
      |ORDER BY h1.z, h1.y, h1.x
      |""".stripMargin)

  // User defined function to calculate cell count of adjacent cells
  var calculateAdjacentCellCount = udf((minX: Int, minY: Int, minZ: Int, maxX: Int, maxY: Int, maxZ: Int, X: Int, Y: Int, Z: Int) =>
    HotcellUtils.calculateAdjacentHotCells(minX, minY, minZ, maxX, maxY, maxZ, X, Y, Z))
  var adjacentHotCells = adjacentHotCellCounts
    .withColumn("adjacent_hot_cells", calculateAdjacentCellCount(lit(minX), lit(minY), lit(minZ), lit(maxX), lit(maxY), lit(maxZ), col("x"), col("y"), col("z")))

  // User defined function to calculate G (Getis-Ord) based on the calculated information
  var calculateGScore = udf((numCells: Int , x: Int, y: Int, z: Int, adjacentHotCells: Int, cellCount: Int, avg: Double, stdDev: Double) =>
    HotcellUtils.calculateGScore(numCells, x, y, z, adjacentHotCells, cellCount, avg, stdDev))
  var hotCellsWithGScore = adjacentHotCells
    .withColumn("g_score", calculateGScore(lit(numCells), col("x"), col("y"), col("z"), col("adjacent_hot_cells"), col("cell_count"), lit(averageHotCells), lit(standardDeviationHotCells)))
    .orderBy(desc("g_score")).limit(50)

  val finalPickupInfo = hotCellsWithGScore.select(col("x"), col("y"), col("z"))

  return finalPickupInfo
